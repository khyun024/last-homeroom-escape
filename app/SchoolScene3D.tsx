...
export function NeonCorridor({ active, onProgress, onDanger, onEscape, onCaught }: Props) {
  ...
  useEffect(() => {
    ...
    box([10,.12,110],[0,-.06,-47],0x121727);
    box([10,.08,110],[0,5.1,-47],0x101321);
    box([.12,5.2,110],[-5,2.5,-47],0x4d7794,true);
    box([.12,5.2,110],[5,2.5,-47],0x4d7794,true);
    ...
    const leftDoor=box([2.18,4.1,.14],[-1.12,2.05,-98],0xc0efff,true);
    const rightDoor=box([2.18,4.1,.14],[1.12,2.05,-98],0xc0efff,true);
    const exitGlow=new THREE.PointLight(0xb4f7ff,120,25); exitGlow.position.set(0,2.2,-100); scene.add(exitGlow);
    ...
    const draw=(now:number)=>{
      ...
      if(active&&!ended){
        const horizontal=(keys.has("ArrowRight")||keys.has("KeyD")?1:0)-(keys.has("ArrowLeft")||keys.has("KeyA")?1:0);
        x=THREE.MathUtils.clamp(x+horizontal*dt*5.5,-3.45,3.45); z-=dt*7.6;
        let collision=false;
        for(const obstacle of obstacles) if(Math.abs(z-obstacle.position.z)<.55&&Math.abs(x-obstacle.position.x)<1.42) collision=true;
        threat+= (collision?42:-2.2)*dt; threat=THREE.MathUtils.clamp(threat,10,100);
        cb.current.onProgress(Math.min(100,Math.round((5-z)/1.03))); cb.current.onDanger(threat);
        if(navigator.vibrate&&collision) navigator.vibrate(25);
        if(threat>=99){ ended=true; cb.current.onCaught(); }
        if(z<=-96){ ended=true; cb.current.onEscape(); }
      }
      camera.position.x+=(x-camera.position.x)*.13; camera.position.z=z; camera.position.y=1.58+Math.sin(t*12)*.035;
      camera.lookAt(camera.position.x*.45,1.6,z-13); runnerLight.position.set(camera.position.x,2.2,z-1);
      hunter.position.set(camera.position.x*.35,0,z+4.5-threat*.028); hunter.rotation.y=Math.PI;
      if(z<-88){ const opening=THREE.MathUtils.clamp((-88-z)/8,0,1); leftDoor.position.x=-1.12-opening*1.6; rightDoor.position.x=1.12+opening*1.6; }
